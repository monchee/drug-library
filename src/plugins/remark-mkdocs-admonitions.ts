...
export const ADMONITION_TYPE_MAP: Record<string, 'note' | 'tip' | 'caution' | 'danger'> = {
  info: 'note',
  note: 'note',
  tip: 'tip',
  warning: 'caution',
...
const iconPaths: Record<'note' | 'tip' | 'caution' | 'danger', P[]> = {
  note: [
    s('path', {
...
export function createAsideNode(type: string, title?: string, bodyChildren: any[] = []): P {
  const normType = (type || '').toLowerCase().trim();
  const variant = ADMONITION_TYPE_MAP[normType] || 'note';
  const finalTitle = (title !== undefined && title !== null && String(title).trim() !== '')
    ? String(title).trim()
    : (DEFAULT_TITLE_MAP[normType] || 'Note');

  const titleParsed = fromMarkdown(finalTitle, {
    extensions: [gfm()],
    mdastExtensions: [gfmFromMarkdown()],
  });
  const titleNodes = titleParsed.children[0]?.type === 'paragraph'
    ? titleParsed.children[0].children
    : [{ type: 'text', value: finalTitle }];
  ...
  return h(
    ...
    [
      ...
      h('div', { class: 'starlight-aside__content' }, bodyChildren),
    ]
  );
}
...
export function scanAdmonitions(content: string): AdmonitionBlock[] {
  ...
  while (i < lines.length) {
    ...
    const matchTripleSlash = line.match(/^\/{3}\s*([a-zA-Z]+)(?:\s*\|\s*(.*))?$/);
    ...
    if (match) {
      ...
      while (j < lines.length) {
        ...
        if (cur.startsWith('    ') || cur.startsWith('\t')) {
          bodyLines.push(cur.startsWith('    ') ? cur.slice(4) : cur.slice(1));
          j++;
        } else if (cur.trim() === '') {
          let k = j + 1;
      ...
      i = j;
    } else if (matchTripleSlash) {
      const startLine = i + 1;
      const type = matchTripleSlash[1];
      const title = matchTripleSlash[2] ?? '';
      const bodyLines: string[] = [];
      let j = i + 1;
      while (j < lines.length) {
        const cur = lines[j];
...
export function transformTreeAdmonitions(tree: any, content: string): void {
  const blocks = scanAdmonitions(content);
  if (blocks.length === 0) return;

  for (const block of blocks) {
    const parsedBody = fromMarkdown(block.body, {
      extensions: [gfm()],
      mdastExtensions: [gfmFromMarkdown()],
    });
    const aside = createAsideNode(block.type, block.title, parsedBody.children);
    ...
    for (let i = 0; i < tree.children.length; i++) {
      ...
      if (node.position) {
        if (node.position.start.line >= block.startLine && node.position.start.line <= block.endLine) {
          if (startIdx === -1) {
            startIdx = i;
          }
          count++;
    ...
    if (startIdx !== -1 && count > 0) {
      tree.children.splice(startIdx, count, aside);
    }
  }
}

export function transformRemainingAdmonitions(tree: any): void {
  visit(tree, (node: any, index: number | undefined, parent: any) => {
    if (!parent || index === undefined) return;
    if (node.type === 'paragraph' && Array.isArray(node.children) && node.children.length > 0) {
      const firstChild = node.children[0];
      if (firstChild.type === 'text' && typeof firstChild.value === 'string' && firstChild.value.startsWith('!!!')) {
        const text = firstChild.value;
        const match = text.match(/^!!!\s+([a-zA-Z]+)(?:\s+(?:"([^"]*)"|'([^']*)'|([^\n]+)))?(?:\n([\s\S]*))?$/);
        if (match) {
          const type = match[1];
          const title = match[2] ?? match[3] ?? match[4] ?? '';
          const bodyFirstLine = match[5] ?? '';
          firstChild.value = bodyFirstLine;
          if (firstChild.value === '') {
            node.children.shift();
          }
          const aside = createAsideNode(type, title, [node]);
...
export function remarkMkdocsAdmonitions() {
  return (tree: any, file: any) => {
    let content = typeof file.value === 'string' ? file.value : '';
    ...
    if (content) {
      transformTreeAdmonitions(tree, content);
    }
    transformRemainingAdmonitions(tree);
